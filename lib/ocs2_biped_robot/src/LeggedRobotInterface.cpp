...
#include "ocs2_biped_robot/LeggedRobotPreComputation.h"
#include "ocs2_biped_robot/constraint/FrictionConeConstraint.h"
#include "ocs2_biped_robot/constraint/NormalVelocityConstraintCppAd.h"
#include "ocs2_biped_robot/constraint/ZeroForceConstraint.h"
#include "ocs2_biped_robot/constraint/ZeroVelocityConstraintCppAd.h"
#include "ocs2_biped_robot/cost/LeggedRobotStateInputQuadraticCost.h"
#include "ocs2_biped_robot/dynamics/LeggedRobotDynamicsAD.h"
...
void LeggedRobotInterface::setupOptimalConrolProblem(const std::string& taskFile, const std::string& urdfFile,
  ...
  std::cerr << "[LeggedRobotInterface] Pinocchio model loaded. " << pinocchioInterfacePtr_->getModel().nq - 6 << " with joints " << pinocchioInterfacePtr_->getModel().njoints << std::endl;
  std::cerr << "Reference File: " << referenceFile << std::endl;
  for (auto& joint_name : pinocchioInterfacePtr_->getModel().names) {
    std::cerr << "Joint Name: " << joint_name << std::endl;
  }
  // CentroidalModelInfo
  ...
  std::cerr << "[LeggedRobotInterface] Centroidal model info created." << std::endl;

  // Swing trajectory planner
  auto swingTrajectoryPlanner =
      std::make_unique<SwingTrajectoryPlanner>(loadSwingTrajectorySettings(taskFile, "swing_trajectory_config", verbose), 4);

  std::cerr << "[LeggedRobotInterface] Swing trajectory planner created." << std::endl;
  ...
  std::cerr << "[LeggedRobotInterface] Mode schedule manager created." << std::endl;
...
matrix_t LeggedRobotInterface::initializeInputCostWeight(const std::string& taskFile, const CentroidalModelInfo& info) {
  ...
  std::cout << "R Size: " << R_taskspace.rows() << ", " << R_taskspace.cols() << std::endl;
