...
vector_t FootPlacementConstraint::getValue(
  ...
  scalar_t middle = referenceManagerPtr_->getGaitSchedule()->getMiddleOfLastStance(time);
...
FootPlacementConstraint::getLinearApproximation(
  ...
  linearApproximation.setZero(numConstraints, state.rows(), input.rows());
  linearApproximation.f = A * positionLinearApproximation.f + b + s;
  linearApproximation.dfdx = A * positionLinearApproximation.dfdx;
